...
public class PostController {
  ...
  @ResponseBody
  public ResponseEntity<String> savePost(PostRequestDto requestDto,
      @RequestParam MultipartFile file) throws IOException {
    boolean httpState = postService.post(requestDto, file);
    if (httpState) {
      return new ResponseEntity<>("글이 성공적으로 작성되었습니다.",
          HttpStatus.CREATED); /* http state code 201 반환 */
    } else {
      return new ResponseEntity<>("글 작성에 실패했습니다.",
          HttpStatus.BAD_REQUEST); /* http state code 400 반환 */
    }
  }
  ...
  public ResponseEntity<String> Auth(
    ...
    if (memberId != null) {
      ...
      return new ResponseEntity<>("토큰이 유효하지 않습니다",
          HttpStatus.BAD_REQUEST); /* http state code 400 반환 */
    }
  }
  ...
  public ResponseEntity<String> saveComment(PostCommentRequestDto postCommentRequestDto) {
    ...
    if (isPost) {
      return new ResponseEntity<>("댓글이 성공적으로 작성되었습니다.",
          HttpStatus.CREATED); /* http state code 201 반환 */
    } else {
      return new ResponseEntity<>("댓글 작성에 실패했습니다.",
          HttpStatus.BAD_REQUEST); /* http state code 400 반환 */
    }
  }
  ...
  public @ResponseBody byte[] GetProfileImage(@PathVariable Long id) throws IOException {
    ...
    System.out.println("memberId: " + post.getMember().getMemberId());
      if (!imageStore.checkBgImg(fileName)) {
          imgRootPath =
              "static/members/" + post.getMember().getMemberId() + "/posts/" + post.getTitle()
                  + "/img";
      }
