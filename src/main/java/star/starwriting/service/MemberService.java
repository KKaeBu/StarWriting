...
public class MemberService {
  ...
  public MemberService(MemberRepository memberRepository, PostRepository postRepository,
      MemberProfileImageRepository memberProfileImageRepository, DirManager dirManager,
      ImageStore imageStore, JwtProvider jwtProvider) {
  ...
  public String Login(String inputMemberID, String inputPassword) {
    ...
    System.out.println(
        "Bcrypt 비밀번호 대조 결과: " + bCryptPasswordEncoder.matches(inputPassword, hashedPassword));
  ...
  public List<Member> follow(FollowRequestDto followRequestDto) {
    Member followedMember = memberRepository.findByMemberId(followRequestDto.getFollowedMemberId())
        .get();
    Member followingMember = memberRepository.findByMemberId(
        followRequestDto.getFollowingMemberId()).get();
